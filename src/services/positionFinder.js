 ...
 */
function simulateMatch(table, yourTeamName, oppTeamName, 
                       yourRuns, yourOvers, oppRuns, oppOvers) {
  ...
  // Update both teams
  const updatedYourTeam = updateTeamAfterMatch(yourTeam, yourRuns, yourOvers, oppRuns, oppOvers, yourTeamWon);
  const updatedOppTeam = updateTeamAfterMatch(oppTeam, oppRuns, oppOvers, yourRuns, yourOvers, !yourTeamWon);
 ...
 */
export function findRunsToRestrict(table, yourTeamName, oppTeamName, 
                                    desiredPosition, runsScored, overs) {
  let minRuns = 0;
  ...
  for (let runs = minRuns; runs <= maxRuns; runs++) {
    const simulatedTable = simulateMatch(
      table, yourTeamName, oppTeamName, 
      runsScored, overs, runs, overs
    );
 ...
 */
export function findOversToChase(table, yourTeamName, oppTeamName, 
                                  desiredPosition, runsToChase, totalOvers) {
  let minOvers = 0.1;
  ...
  for (let overs = minOvers; overs <= maxOvers; overs += 0.1) {
    const simulatedTable = simulateMatch(
      table, yourTeamName, oppTeamName, 
      runsToChase, overs, runsToChase - 1, totalOvers
    );
 ...
 */
export function calculateRequiredPerformance(table, yourTeamName, oppTeamName, 
                                              desiredPosition, tossResult, runs, overs) {
  if (tossResult === 'batting') {
    // Batting first: find runs to restrict
    return findRunsToRestrict(table, yourTeamName, oppTeamName, desiredPosition, runs, overs);
  } else {
    // Bowling first: find overs to chase in
    return findOversToChase(table, yourTeamName, oppTeamName, desiredPosition, runs, overs);
  }
}